...
class CurriculumBuffer(Buffer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.mode_env not in [0, 1]:
            raise Exception(f"Mode {self.mode_env} of the environment is not implemented.")
        if self.mode_target not in [0, 1]:
            raise Exception(f"Mode {self.mode_target} of the targets is not implemented.")
    ...
    def _curriculum_step(self, num_envs=3, velocities=None, length_percentages=None, angles=None, steps_per = 0):
        ...
        if length_percentages is None:
            raise Exception("length_percentage cannot be None to perform a curriculum step.")
        if angles is None:
            raise Exception("angles cannot be None to perform a curriculum step.")
        if velocities is None:
            raise Exception("velocities cannot be None to perform a curriculum step.")
        ...
        if self.mode_target == 0:
            vel_percent_diffs = [abs(velocity[0] - velocity[1]) / (velocity[1]+0.0001) for velocity in velocities]
            angle_percent_diffs = [abs(angle[0] - angle[1]) / np.pi for angle in angles]
            
            vel_percent_diff = np.mean(vel_percent_diffs[:int(len(vel_percent_diffs) * 3 / 4)])
            angle_percent_diff = np.mean(angle_percent_diffs[:int(len(angle_percent_diffs) * 3 / 4)])
            ...
            if angle_percent_diff <= target_0_threshold[1]:
                angle_percent = min(np.pi, self.last_angle_range[1] + np.pi / 32)
                self.last_angle_range = (-angle_percent, angle_percent)

        elif self.mode_target == 1:
            if steps_per >= target_1_threshold[1]:
                angle_percent = min(1, (steps_per - target_1_threshold[1]) / 0.2)
                self.last_angle_range = (-angle_percent * np.pi, angle_percent * np.pi)
                self.last_task = 2
            
            if steps_per >= target_1_threshold[0]:
                vel_percent = min(1, (steps_per - target_1_threshold[0]) / 0.2)
                self.last_vel_range = (1 - 0.75 * vel_percent, 1 + 0.25 * vel_percent)
                self.last_task = 1
                                       
            if steps_per >= target_1_threshold[2]:
                self.last_stand_prob = 1 - 0.95 * min(1, (steps_per - target_1_threshold[2]) / 0.2)
                self.last_task = 0
        
        return self.last_env_index, self.last_angle_range, self.last_vel_range, self.last_stand_prob, self.last_task
        
        
        
    def _get_env_index(self, num_envs = 3, velocities = None, length_percentages = None, angles = None, steps_per = 0):
        ...
        if self.no_switch > 0:
            ...
            percent_diffs = [abs(velocity[0] - velocity[1]) / velocity[1] for velocity in velocities]
            # take the mean of the lowest 3/4 of the percentages
            percent_diff = np.mean(sorted(percent_diffs)[:int(len(percent_diffs) * 3 / 4)])
            ...
            percent_diffs = [abs(angles[0] - angles[1]) / angles[1] for angles in angles]
            # take the mean of the lowest 3/4 of the percentages
            percent_diff = np.mean(sorted(percent_diffs)[:int(len(percent_diffs) * 3 / 4)])
            ...
            vel_percent_diffs = [abs(velocity[0] - velocity[1]) / velocity[1] for velocity in velocities]
            angle_percent_diffs = [abs(angle[0] - angle[1]) / angle[1] for angle in angles]
            
            vel_percent_diff = np.mean(sorted(vel_percent_diffs)[:int(len(vel_percent_diffs) * 3 / 4)])
            angle_percent_diff = np.mean(sorted(angle_percent_diffs)[:int(len(angle_percent_diffs) * 3 / 4)])
